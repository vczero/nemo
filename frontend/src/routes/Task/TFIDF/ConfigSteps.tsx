...
import { Steps, Input, InputNumber, Button, Upload, Checkbox, App, Result } from 'antd'
import {
  UploadOutlined,
  FileTextOutlined,
  DownloadOutlined,
...
interface ConfigStepsProps {
  config: TTfIdfTaskConfigResponse | null
  configLoading: boolean
  onSubmit: (params: {
    inputFiles?: TInputFile[]
    taskName?: string
    taskParams?: TTfIdfTaskParams
  }) => Promise<string | null>
}
...
export default function ConfigSteps({ config, onSubmit }: ConfigStepsProps) {
  ...
  const [stopwordsFileName, setStopwordsFileName] = useState<string | null>(null)
      ...
      {currentStep === 0 && (
        <div>
          <div className="mb-6 rounded-sm border border-gray-200 bg-white p-4">
            <h3 className="mb-2 text-base font-bold">需要进行处理的文本</h3>
            <TextArea
              rows={6}
              placeholder="请输入需要处理的文本；每一行一个文档；超过 10,000 字请上传文件。"
              value={textContent}
              onChange={(e) => {
            ...
            <div className="my-2 flex flex-wrap gap-3">
              ...
              <Button
                ...
                type="default"
                onClick={() =>
                  createAndDownloadTemplateFile('TF-IDF_数据模板.xlsx')
                }
              >
                下载样例数据
              </Button>
            ...
            <FileAlert keyword="待处理的原始文本，每一行一篇文本" className="mt-2" />
          </div>
          ...
          <div className="mb-6 rounded-sm border border-gray-200 bg-white p-4">
            ...
            <div className="my-2 flex flex-wrap gap-3">
              ...
              <Button
                ...
                type="default"
                onClick={() =>
                  createAndDownloadTemplateFile('TF-IDF_停用词模板.xlsx', [
                    ['id', 'text'],
                    [1, '停用词 1'],
                    [2, '停用词 2'],
                  ])
