...
import { Steps, Input, Slider, Button, Upload, App } from 'antd'
import {
  UploadOutlined,
  FileTextOutlined,
  DownloadOutlined,
...
import type { TSentimentTaskParams, TInputFile } from '@/apis/ml_task'
...
interface ConfigStepsProps {
  ...
  onSubmit: (params: {
    ...
    taskName?: string
    taskParams?: TSentimentTaskParams
  }) => Promise<string | null>
}
...
export default function ConfigSteps({ onSubmit }: ConfigStepsProps) {
  ...
  // Sentiment thresholds: [negativeUpper, positiveLower]
  // 消极: 0 → thresholds[0], 中性: thresholds[0] → thresholds[1], 积极: thresholds[1] → 1
  const [thresholds, setThresholds] = useState<[number, number]>([0.4, 0.6])
    ...
    try {
      ...
      await onSubmit({
        ...
        taskParams: {
          ...
          neutral_threshold: thresholds[1],
  ...
  return (
    <div>
      ...
      <div className="mb-6 rounded-sm border border-gray-200 bg-white p-4">
        <h3 className="mb-1 text-base font-bold">需要进行情感计算的文本</h3>
        <p className="mb-3 text-sm text-gray-400">
          情感计算默认使用的 SnowNLP 模型
        </p>

        <TextArea
          rows={6}
          placeholder="请输入需要分析的文本，例如新闻、评论等；按照每一行进行情感计算任务；超过 10,000 字请上传文件。"
          value={textContent}
          onChange={(e) => {
        ...
        <div className="my-2 flex flex-wrap gap-3">
          ...
          <Button
            ...
            type="default"
            onClick={() =>
              createAndDownloadTemplateFile('情感计算_数据模板.xlsx')
            }
          >
            下载模板
          </Button>
      ...
      <div className="mb-6 rounded-sm border border-gray-200 bg-white p-4">
        <h3 className="mb-3 text-base font-bold">情感值范围设置</h3>
        <Slider
          range
          min={0}
          max={1}
          step={0.01}
          value={thresholds}
          onChange={(v) => setThresholds(v as [number, number])}
        ...
        <div className="mt-2 flex justify-between text-sm">
          <span className="text-red-500">消极: 0 - {thresholds[0]}</span>
          <span className="text-gray-500">
            中性: {thresholds[0]} - {thresholds[1]}
          </span>
          <span className="text-green-600">积极: {thresholds[1]} - 1</span>
        </div>
      </div>
