...
import { textClassificationReport } from './charts'
import type { TMLTaskResultResponse } from '@/apis/ml_task'
import type { TASK_TYPES } from '@/constants/ml_task'
import { downloadFile } from '@/utils/utils'
import ReportAlert from '../components/ReportAlert'

type TextClassificationResult = TMLTaskResultResponse<
  typeof TASK_TYPES.TEXT_CLASSIFICATION
>
...
interface TextClassificationRow {
  [key: string]: unknown
  id: string | number
  text: string
  label: string
...
const gridColumns: Column<TextClassificationRow>[] = [
  {
    key: 'id',
    name: '序号',
    resizable: true,
...
export default function Report({ result, onNewTask }: ReportProps) {
  ...
  const tableData = (tableState?.data ?? []) as unknown as TextClassificationRow[]
  ...
  const handleDownloadData = (url?: string, fileSuffix?: string) => {
    if (!url) return
    downloadFile(
      `文本分类_${result.taskId}${fileSuffix ? `_${fileSuffix}` : ''}.xlsx`,
      url,
    )
  }

  const pieState = charts.get(textClassificationReport.charts[0].key)
    ...
    >
      <div className="mb-6">
        <ReportChartCard
          ...
          containerRef={bindRef(textClassificationReport.charts[0].key)}
          maxWidth="480px"
          loading={
