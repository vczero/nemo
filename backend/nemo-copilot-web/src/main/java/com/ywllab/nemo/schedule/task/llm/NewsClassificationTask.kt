...
class NewsClassificationTask(
    ...
    override fun buildSystemPrompt(): String {
        ...
        return buildString {
            appendLine("你是一名专业的新闻标注员，擅长新闻分类任务。")
            appendLine("**类别列表**：")
            appendLine(categoryList)
            if (isMultiClass) {
                appendLine("每条新闻可属于多个类别。")
                appendLine("输出：jsonlines格式，每行{\"id\":\"xxx\",\"label\":\"类别1@类别2\"}")
            } else {
                appendLine("每条新闻只能属于一个类别。")
