...
def compute_frequent_itemset(user_data_oss_path, user_selected_stopword_names, user_stopwords_path, min_support=0.1):
    try:
    ...
    def tokenize(text):
        ...
        for word in jieba.cut(text):
            if word.strip() and word not in ALL_STOPWORDS_SET and len(word) > 1:
                words.append(word)
        return words

    transactions = [tokenize(doc) for doc in documents]
    ...
    result_all = upload_excel_data(results_df.head(10000), 'frequent_itemset')
    ...
    CHART_single_RE = upload_excel_data(CHART_single, 'frequent_itemset')
    ...
    CHART_multi_RE = upload_excel_data(CHART_multi, 'frequent_itemset')
    ...
    result_summary = upload_excel_data(summary_df, 'frequent_itemset')
